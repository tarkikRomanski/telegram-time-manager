...
export async function list(telegramId, userId) {
    ...
    if (!listOfTasks || !listOfTasks.length) {
        await store.bot.sendMessage(telegramId, `${EMOJI.no_entry} List of tasks is empty!`);
        return [];
    }
    ...
    await store.bot.sendMessage(
        telegramId,
        listOfTasks.map(task => `${EMOJI.balloon} ${bold(task.taskId)}: ${task.name || 'Untitled'}`).join('\n\n'),
        opts
    );
